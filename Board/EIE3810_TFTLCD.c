...
void EIE3810_TFTLCD_ShowChar(u16 x, u16 y, u8 ASCII, u16 color, u16 bgcolor){
    u8 i,j,k;
    if(ASCII <32 || ASCII == 127) return; //no print
    for(i=0;i<8;i++){
        for(j=0;j<8;j++){
...
void EIE3810_TFTLCD_Init(void)
{
  ...
  RCC->APB2ENR|=1<<3;     	//Enable PORTB clock
	...
	FSMC_Bank1->BTCR[6]=0X00000000; // FSMC_Bank1->BTCR[6]= FSMC_BCR4
	FSMC_Bank1->BTCR[7]=0X00000000; // FSMC_Bank1->BTCR[7]= FSMC_BTR4
	FSMC_Bank1E->BWTR[6]=0X00000000; // FSMC_Bank1E->BWTR[6]= FSMC_BWTR4

	FSMC_Bank1->BTCR[6]|=1<<12;		// Enable write operations for the bank
	FSMC_Bank1->BTCR[6]|=1<<14;		// Extended mode enable, enable different timing for read an write
	                              // (since read time < write time)
	FSMC_Bank1->BTCR[6]|=1<<4; 		// Memory databus width -> 16 bit
						    
	FSMC_Bank1->BTCR[7]|=0<<28;		// Access mode A
	FSMC_Bank1->BTCR[7]|=1<<0; 		// Set duation of the first access phase for read access: 2 HCLK cycles
	FSMC_Bank1->BTCR[7]|=0XF<<8;  	// Set duration of the second access phase for read access: 16 HCLK cycles
	
	FSMC_Bank1E->BWTR[6]|=0<<28; 	// Access mode A
	FSMC_Bank1E->BWTR[6]|=0<<0;		// Set duration of the first access phase for write access: 1 HCLK cycles
 	FSMC_Bank1E->BWTR[6]|=3<<8; 	// Set duration of the second access phase for write acess: 4 HCLK cycles
	
	FSMC_Bank1->BTCR[6]|=1<<0;		// Bank 4 enabled
...
// Draw a circle (hollow or filled)
...
// Show a single character in 24x12 font
